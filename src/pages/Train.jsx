import { Dimensions, Image, Pressable, ScrollView, StyleSheet, Text, TextInput, useColorScheme, useWindowDimensions, View } from "react-native";
import { Header } from "../components/Header";
import { useState } from "react";
import DateTimePicker from "react-native-ui-datepicker";
import { useSelector } from "react-redux";
import { scheme } from "../components/Assets";
...
export const Train = ({navigation})=>{
	...
	<>
		<Header title={'Trains'}/>
		<ScrollView style={{...styles.pageContainer,backgroundColor:scheme[useColorScheme()]}}>
			<View style={{...styles.block,backgroundColor:scheme[useColorScheme()]}}>

				<Text style={styles.heading}>Select Station</Text>
				<View style={styles.trainStation}>
					<View style={styles.stationname}>
						<Text style={styles.subTitle}>From</Text>
						<TextInput style={styles.stationInput} value={source} onChange={(e)=>setSource((e.nativeEvent.text).toLocaleUpperCase())} placeholder="Source station" placeholderTextColor={'grey'}></TextInput>
					</View>
					...
					<View style={styles.stationname}>
						...
						<TextInput style={styles.stationInput} value={destination} onChange={(e)=>setDestination((e.nativeEvent.text).toLocaleUpperCase())} placeholder="Destination station" placeholderTextColor={'grey'}></TextInput>
					</View>
				</View>
				...
				<Pressable onPress={()=>setShowResults(!showResults)} android_ripple={{color:'#AC0B0B',radius:200,foreground:true}}>
					<Text style={styles.submitBtn}>Search Trains</Text>
				</Pressable>
			</View>
